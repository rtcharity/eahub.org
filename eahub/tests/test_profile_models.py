...
from eahub.profiles.models import CauseArea, Profile, ProfileAnalyticsLog, Membership
...
class ProfileTestCase(TestCase):
    ...
    def test_save_analytics_on_profile_creation(self):
        profile = create_profile("test@email.com", "User1")
        local_group = LocalGroup()
        local_group.name = "EA Klaipeda"
        local_group.save()

        m = Membership(profile=profile, local_group=local_group)
        m.save()
        print(profile.local_groups)
        ...
        analytics_logs_local_groups = ProfileAnalyticsLog.objects.filter(
            profile=profile, field="local_groups"
        )
        ...
        self.assertEqual(str(["EA Klaipeda"]), analytics_logs_local_groups[0].value)
    ...
    def test_save_analytics_on_change(self):
        ...
        self.assertEqual(str(["Meta"]), analytics_logs_cause_area_updated[0].value)
