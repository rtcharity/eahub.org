...
class ProfileTestCase(EAHubTestCase):
    def test_private_profile_hidden(self):
        profile = self.gen.profile(first_name="first", last_name="last", visibility=VisibilityEnum.PRIVATE)
        profile.save()
    ...
    def test_public_profile_visible(self):
        profile = self.gen.profile(first_name="first", last_name="last", visibility=VisibilityEnum.PUBLIC)
        profile.save()
        response = self.client.get(reverse("profiles_app:profile", args=([profile.slug])))
        self.assertEqual(response.status_code, 200)
