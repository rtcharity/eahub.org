...
class ReportAbuseForm(forms.Form):
    ...
    reasons = forms.MultipleChoiceField(choices=CHOICES, widget=forms.CheckboxSelectMultiple)

    def clean(self):
        cleaned_data = super(ReportAbuseForm, self).clean()
        if not cleaned_data['reasons']:
            self._errors["reasons"] = [cleaned_data['reasons']] # Will raise a error message
        else:
